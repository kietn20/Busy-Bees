...
export default function EditGroupForm({
  ...
  const [group, setGroup] = useState<InitialGroupShape | null>(
    initialGroup || null
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!mounted) return;
        setGroup({
          id: (g as any)._id || undefined,
          groupName: g.groupName,
          description: g.description,
          owner: (g as any).ownerId,
        });
        setName(g.groupName || "");
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await updateCourseGroup(idToUse, {
        groupName: name.trim(),
  ...
  return (
    <div className="p-6">
      <h2 className="text-lg font-semibold mb-6 border-b pb-2">Profile</h2>
      <form
        onSubmit={(e) => {
          e.preventDefault();
          handleSave();
        ...
        <div className="flex flex-col">
          <div className="space-y-2 mx-4 my-2 py-2">
            ...
            <Input
              ...
              className="bg-white border-foreground/10 focus:border-foreground/30 "
            />
          </div>
