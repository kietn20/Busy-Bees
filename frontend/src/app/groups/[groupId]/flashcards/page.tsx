...
export default function FlashcardsList() {
	...
	useEffect(() => {
		...
		const fetchSets = async () => {
			...
			try {
				const response = await getFlashcardSetsByGroup(
					groupId as string
				);
	...
	const toggleFavorite = async (setId: string, next: boolean) => {
		try {
			if (next)
				await addFavorite(groupId as string, "flashcardSet", setId);
			else await removeFavorite(groupId as string, "flashcardSet", setId);
			setFavoritesMap((m) => ({ ...m, [setId]: next }));
			if (next)
				toast.success("Flashcard Set Saved to Favorites.", {
					id: "favorite-toast",
				});
			else
				toast.success("Flashcard Set removed from favorites.", {
					id: "favorite-toast",
				});
		} catch (err) {
			console.error(
				"toggleFavorite (flashcard) error:",
				err?.response ?? err
			);
			const msg =
	...
	const filteredSets = useMemo(() => {
		...
		result.sort((a, b) => {
			...
			const aDate = new Date(
				anyA.updatedAt || anyA.createdAt || 0
			).getTime();
			const bDate = new Date(
				anyB.updatedAt || anyB.createdAt || 0
			).getTime();
	...
	return (
		<div className="container mx-auto p-8">
			<div className="flex items-center justify-between mb-4">
				...
				<Button
					onClick={() =>
						router.push(`/groups/${groupId}/flashcards/create`)
					}
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
					{filteredSets.map((flashcard) => (
						<FlashcardInfo
							...
							onClick={() =>
								handleFlashcardSetClick(flashcard._id)
							}
							onDelete={() =>
								setFlashcardsData((prev) =>
									prev.filter(
										(set) => set._id !== flashcard._id
									)
								)
							}
							favorited={Boolean(favoritesMap[flashcard._id])}
							onToggleFavorite={(next) =>
								toggleFavorite(flashcard._id, next)
							}
						/>
					))}
				</div>
